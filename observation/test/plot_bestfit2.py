...
# if hdul.__contains__('model:total'):
    ...
    # mask=total['flux']>0
    # plt.plot(total[mask]['wavelength_rest'],total[mask]['flux'],label='Total')
...
# for model in models:
    ...
    # plt.plot(obs_phot[mask]['wavelength_rest'],obs_phot[mask]['flux'],label=model.replace('model:',''))
    # if len(models)==1:
...
if hdul.__contains__('obs:spec') and '--no_spectra_fit' not in hdul[0].header['RUN_ARGUMENTS']:
    ...
    for i in bands:
        plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],obs_spec[obs_spec['iband']==i]['spectra_obs'],label='spec:obs_'+str(i))
        plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],obs_spec[obs_spec['iband']==i]['spectra_mod'],label='spec:mod_'+str(i))
        plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],np.abs(obs_spec[obs_spec['iband']==i]['spectra_obs']-obs_spec[obs_spec['iband']==i]['spectra_mod']),label='spec:diff_'+str(i))
        # plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],c1*obs_spec[obs_spec['iband']==i]['wavelength_rest']**-2*obs_spec[obs_spec['iband']==i]['spectra_obs'],label='spec:obs_'+str(i))
        # plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],c1*obs_spec[obs_spec['iband']==i]['wavelength_rest']**-2*obs_spec[obs_spec['iband']==i]['spectra_mod'],label='spec:mod_'+str(i))
        # plt.plot(obs_spec[obs_spec['iband']==i]['wavelength_rest'],c1*obs_spec[obs_spec['iband']==i]['wavelength_rest']**-2*obs_spec[obs_spec['iband']==i]['spectra_obs']-c1*obs_spec[obs_spec['iband']==i]['wavelength_rest']**-2*obs_spec[obs_spec['iband']==i]['spectra_mod'],label='spec:diff_'+str(i))
        ...
        plt.axvline(x=0.2798, color='brown', linestyle='--',label='MgII2798 ',linewidth=0.5)
...
plt.title("ID="+hdul[0].header['ID']+"\nSNR="+hdul[0].header['SNR']+",z_best="+hdul[0].header['z_{MAL}']+",Xmin^2/Nd="+hdul[0].header['XMIN^2/ND'])
